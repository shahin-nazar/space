...
@export var ab_error_scale: float = 0.2
...
var update_interval := 1.0 / 15.0 # 15 times a second, not every frame
...
func _ready() -> void:
	...
	var shader_file: RDShaderFile = load("res://postprocessing/dither-costly/dither_diffusion.glsl")
	...
	_build_palette()
	print("palette_count = ", palette_count)
	for i in range(palette_hex.size()):
		var rgb01 = hex_to_rgb01(palette_hex[i])
		print(i, ": ", palette_hex[i], " -> rgb01=", rgb01, " oklab=", rgb01_to_oklab(rgb01))
...
func _process(delta: float) -> void:
	...
	var vp_rid: RID = source_viewport.get_viewport_rid()
	var vp_tex_rid: RID = RenderingServer.viewport_get_texture(vp_rid)
	var src_rid: RID = RenderingServer.texture_get_rd_texture(vp_tex_rid)
	print("src_rid valid: ", src_rid.is_valid())
